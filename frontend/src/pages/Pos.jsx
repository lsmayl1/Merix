...
export const Pos = () => {
    ...
    columnHelper.accessor("quantity", {
      ...
      cell: (value) => (
        <div className="flex items-center gap-2 justify-center">
          <button
            onClick={() =>
              handleChangeQunatity(value.row.original.barcode, "deacrese")
            }
            className="bg-white border border-mainBorder rounded-lg p-0.5"
          >
            <Minus className="size-4 text-black" />
          </button>
          <span className={"text-xl w-4 max-w-4 text-center"}>
            {value.getValue()}
          </span>
          <button
            onClick={() =>
              handleChangeQunatity(value.row.original.barcode, "increase")
            }
            className="bg-white border border-mainBorder rounded-lg p-0.5"
          >
  ...
  const handleChangeQunatity = async (barcode, action) => {
    const existProductBarcode = inputData.find((x) => x.barcode == barcode);
    if (existProductBarcode) {
      ...
      try {
        const validProduct = await trigger(barcode);
        if (!validProduct) {
          return;
        }
        const product = validProduct.data;
        setInputData((prevData) => [
          ...prevData,
          {
            quantity: 1,
  ...
  return (
    <div className="flex flex-col  overflow-hidden h-screen  gap-2 w-full ">
      <BarcodeField
        handleBarcode={(id) => handleChangeQunatity(id, "increase")}
      />
      <div className="flex gap-4 items-center justify-between px-8 py-4">
        ...
        <div className="flex flex-col relative w-1/2">
          ...
          {searchData?.length > 0 && query && (
            <div className="absolute w-full h-[400px] top-12 bg-white rounded-lg border border-mainBorder">
              <ul className="overflow-auto h-full px-4 flex flex-col  ">
                {searchData?.map((item) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        handleChangeQunatity(item.barcode, "increase")
                      }
      ...
      <div className="bg-[#F8F8F8] w-full flex h-full px-4  min-h-0">
        <ProductShortcuts
          data={data?.items}
          // products={products}
          handleChangeQunatity={handleChangeQunatity}
        />
